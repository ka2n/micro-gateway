...
func findServicesMessagesAndEnumerations(s []*descriptor.Service, reg *descriptor.Registry, m messageMap, e enumMap, sc securitiesMap, refs refMap) {
	for _, svc := range s {
...
func renderSecuritiesAsDefinition(securities securitiesMap, d swaggerSecurityDefinitions, reg *descriptor.Registry) {
	for name, sec := range securities {
		d[name] = swaggerSecurityScheme{
			Name:        sec.Name,
			Type:        sec.Type,
			In:          sec.In,
	...
	for svcIdx, svc := range services {
		for methIdx, meth := range svc.Methods {
			for _, b := range meth.Bindings {
				...
				if b.Security != "" {
					operationObject.Security = swaggerSecuritiesObject{map[string]swaggerSecurityScopeObject{b.Security: swaggerSecurityScopeObject{}}}
				}
...
func applyTemplate(p param) (string, error) {
	...
	s := swaggerObject{
		...
		SecurityDefinitions: make(swaggerSecurityDefinitions),
	...
	e := enumMap{}
	sc := securitiesMap{}
	findServicesMessagesAndEnumerations(p.Services, p.reg, m, e, sc, refs)
	renderMessagesAsDefinition(m, s.Definitions, p.reg)
	renderEnumerationsAsDefinition(e, s.Definitions, p.reg)
	renderSecuritiesAsDefinition(sc, s.SecurityDefinitions, p.reg)
