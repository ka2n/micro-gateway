...
import (
	...
	options "github.com/ka2n/micro-gateway/protobuf/go/micro/gw"
)
...
func extractServiceAPIOptions(sd *descriptor.ServiceDescriptorProto) (*options.SecurityRule, error) {
	...
	if !proto.HasExtension(sd.Options, options.E_Security) {
		return nil, nil
	}
	ext, err := proto.GetExtension(sd.Options, options.E_Security)
	if err != nil {
		return nil, err
	}
